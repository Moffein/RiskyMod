			...
			On.RoR2.CharacterBody.Start += (orig, self) =>
			{
				...
				if (Run.instance.gameModeIndex != RiskyMod.simulacrumIndex)
                {
					...
					if (dw && dw.goldReward > 0)
					{
						...
						int goldRewardRaw = (int)Mathf.Max(Mathf.Round(dw.goldReward * chestRatio * inflationRatio * (scaleToDirectorMultiplier ? CombatDirectorMultiplier.scaledGoldDivisor : 1f)), 1f);
