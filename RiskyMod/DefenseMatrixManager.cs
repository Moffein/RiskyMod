...
namespace RiskyMod
{
    //Allows Defense Matrix to block enemy hitscan without blocking friendly attacks.
    //This works by toggling colliders on/off, so the GameObject's layer needs to be something that the hitscan will collide with.
    public static class DefenseMatrixManager
    {
        ...
        public static bool enabled = false; //Only gets enabled when a RiskyMod component is using it.
